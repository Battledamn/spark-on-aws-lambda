...
if [ ! -d /usr/lib/jvm/java-17-amazon-corretto ]; then
  echo "ERROR: /usr/lib/jvm/java-17-amazon-corretto not found. Installed JVMs:" >&2
  ls -la /usr/lib/jvm >&2
  exit 1
fi
...
# the entirety of the following section should NOT be reuqired if assumptions are correct
...
# Download specific statistics implementation jars -- ommiting guava for now
...
echo "export HADOOP_CONF_DIR=${SPARK_HOME}/conf" >> $SPARK_HOME/conf/spark-env.sh


# Add AWS SDK v2components for better S3 compatibility -- should be fixed by iceberg-aws-bundle
